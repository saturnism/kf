...
func (r *Reconciler) ApplyChanges(ctx context.Context, route *v1alpha1.Route) error {
	...
	// If the VirtualService isn't ready (either it's coming up or being
	// deleted) then this reconciliation process can't continue until we get
	// notified it is.
	if cond := route.Status.GetCondition(v1alpha1.RouteConditionVirtualServiceReady); cond != nil {
		if !cond.IsTrue() {
			logger.Infof("can't continue reconciling until VirtualService %q is ready", virtualServiceName)
			return nil
		}
	}
